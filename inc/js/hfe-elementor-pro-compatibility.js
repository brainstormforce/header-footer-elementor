		...
		init() {
			elementor.on( 'document:loaded', function() {
				setTimeout( function() {
					jQuery.each( elementorFrontend.documentsManager.documents, function( index, document ) {
						...
						const ids_array = JSON.parse( hfe_admin.ids_array );
						ids_array.forEach( function( item, index ) {
                        	const elementor_id = $documentElement.data( 'elementor-id' );
			...
			elementor.on( 'panel:init', function() {
				function scrollToTop( changedValue ) {
					const changedItem = Object.entries( this.model.changed )[ 0 ];
				...
				const changeHandler = [ 'hfe_scroll_to_top_global', 'hfe_scroll_to_top_media_type', 'hfe_scroll_to_top_button_icon', 'hfe_scroll_to_top_button_image', 'hfe_scroll_to_top_button_text', 'hfe_scroll_to_top_single_disable' ];
