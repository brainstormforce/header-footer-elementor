( function( $, window, undefined ) {
	const init_target_rule_select2 = function( selector ) {
		$( selector ).astselect2( {
	...
	const update_target_rule_input = function( wrapper ) {
		const rule_input 		= wrapper.find( '.ast-target_rule-input' );
		const old_value = rule_input.val();
		const new_value = [];

		wrapper.find( '.astra-target-rule-condition' ).each( function( i ) {
			const $this 			= $( this );
			let temp_obj 		= {};
			const rule_condition 	= $this.find( 'select.target_rule-condition' );
			const specific_page 	= $this.find( 'select.target_rule-specific-page' );

			const rule_condition_val 	= rule_condition.val();
			const specific_page_val 	= specific_page.val();
		...
		const rules_string = JSON.stringify( new_value );
		rule_input.val( rules_string );
	};

	const update_close_button = function( wrapper ) {
	...
	const update_exclusion_button = function( force_show, force_hide ) {
		const display_on = $( '.ast-target-rule-display-on-wrap' );
		const exclude_on = $( '.ast-target-rule-exclude-on-wrap' );

		const exclude_field_wrap = exclude_on.closest( 'tr' );
		const add_exclude_block = display_on.find( '.target_rule-add-exclusion-rule' );
		const exclude_conditions = exclude_on.find( '.astra-target-rule-condition' );
		...
		} else if ( 1 == exclude_conditions.length && '' == $( exclude_conditions[ 0 ] ).find( 'select.target_rule-condition' ).val() ) {
			exclude_field_wrap.addClass( 'ast-hidden' );
			add_exclude_block.removeClass( 'ast-hidden' );
		} else {
			exclude_field_wrap.removeClass( 'ast-hidden' );
	...
	$( document ).ready( function( $ ) {
		...
		jQuery( '.ast-target-rule-selector-wrapper' ).on( 'click', '.target_rule-add-rule-wrap a', function( e ) {
			...
			rule_wrap.append( template( { id: new_id, type } ) );
