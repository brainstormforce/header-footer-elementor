...
class HFE_Helper {
	...
	public static function premium_starter_templates_status() {
		...
		return Widgets_Config::get_plugin_status( 'astra-pro-sites/astra-pro-sites.php' );
	}
	...
	public static function free_starter_templates_status() {
		return Widgets_Config::get_plugin_status( 'astra-sites/astra-sites.php' );
	}
			...
			if ( is_array( $widgets ) ) {
				...
				foreach ( $widgets as $slug => $data ) {
					...
					if ( isset( $saved_widgets[ $slug ] ) ) {
						...
						$widgets[ $slug ]['is_activate'] = $data['default'] ?? false;
					}
				}
			}
