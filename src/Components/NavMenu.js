...
const NavMenu = () => {
	...
	return (
        ...
        <img src={`${hfeSettingsData.icon_url}`} alt="My Icon" className="h-24 w-24" />
    </Link>
				</Topbar.Item>
			</Topbar.Left>
				...
				<Topbar.Item>
					<nav className="flex gap-2 cursor-pointer text-text-secondary">
						...
						<Link
							...
							className={`text-text-primary ${
								isActive("templates") ? "active-link" : ""
							}`}
							style={{
								borderBottom: isActive("templates")
									? "2px solid #6005FF"
									: "none",
								paddingBottom: "4px",
							}}
