...
const CustomBlock = () => {
					...
					apiFetch({
						...
						.then((refreshResponse) => {
							if (
								...
								refreshResponse.posts
							) {
								setCustomBlockItems(refreshResponse.posts);
								setCustomBlocks(
									refreshResponse.posts.length > 0,
								);
							}
						})
						.catch((error) => {
							console.error(
								"Error refreshing custom blocks:",
								error,
							);
						});
				} else {
	...
	if (!hasCustomBlocks) {
		...
		return (
			<>
				<div
					className="custom-block-section"
						...
						{customBlockItems.map((item) => (
							...
							>
								<div className="relative h-60 w-full">
									...
									<div
										className="hover-overlay absolute inset-0 flex flex-col gap-1 items-center justify-center rounded-lg overflow-hidden backdrop-blur-sm transition-all duration-500 ease-in-out z-30"
										style={{
											backgroundColor:
												"rgba(0, 0, 0, 0.4)",
											opacity: "0",
