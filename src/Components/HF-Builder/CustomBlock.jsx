	...
	const customBlockItems = [
		{
			...
			buttonText: __("Create Custom Block", "header-footer-elementor"),
			onClick: () => window.open("https://your-site.com/edit-custom-block", "_blank"),
		},
		// Add more custom block items as needed
	];

	const handleCreateCustomBlock = (item) => {
		// For custom blocks, you might want to handle creation differently
		if (item.name === "Custom Block" && !item.id) {
			// Direct redirect for new custom blocks without opening dialog
			item.onClick();
			return;
		}
		
		// For existing custom blocks, open the display conditions dialog
						...
						>
							<div className="relative h-60 w-full">
								...
								<div className="absolute inset-0 flex items-center justify-center rounded-lg overflow-hidden backdrop-blur-sm bg-black/40 transition-all duration-200 z-30">
									<Button
										iconPosition="left"
										icon={item.name !== "Custom Block" ? <Plus size={14} /> : null}
										variant="primary"
										...
										onClick={() => handleCreateCustomBlock(item)}
									>
										{item.buttonText}
									</Button>
								</div>
