...
function StepTemplate( { item, mode, step_id } ) {
	...
	return (
		<>
			<div
				key={ item.id }
				className="wcf-template-item group cursor-pointer relative transition ease-in-out delay-150"
			>
				{ 'pro' === item.template_type ? (
					<div
						className={ `wcf-item__type wcf-item__type--${ item.template_type } border border-primary-500 text-primary-500 rounded-lg bg-white font-normal py-px px-2 text-xs z-10 uppercase absolute top-5 right-5 shadow-custom-2` }
					>
						<div className="bg-primary-25 rounded-lg text-primary-600">
							{ item.template_type }
						</div>
					</div>
				) : (
				...
				<div className="wcf-item__inner bg-white border border-gray-200 rounded-lg">
					<div className="wcf-item__thumbnail-wrap relative">
						...
						<div className="wcf-item__view invisible group-hover:visible absolute backdrop-brightness-50 top-0 w-full h-full transition ease-in-out delay-150">
							...
							{ mode === 'dynamic' && (
								<div className="mx-auto my-0 text-center relative top-1/2">
									<UpdateButton
										currentStep={ item }
										stepName={ 'StepName' }
					...
					<div className="wcf-item__heading-wrap border-t border-gray-200">
						<span className="wcf-item__heading block text-base text-gray-800 font-medium p-4 text-center">
							{ item.title }
						</span>
					</div>
				</div>
